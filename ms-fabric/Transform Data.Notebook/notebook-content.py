...
class ResearchPaymentTransformer(GeneralPaymentTransformer):
    def add_investigators_struct(
        self, primary_type_cols: List[str], specialty_cols: List[str]
    ) -> "ResearchDataTransformer":
        """
        Create an array of structs for the investigators with their primary types and specialties.
        Assumes that primary_type_cols and specialty_cols both have a length of 5.
        """
        assert (
            len(primary_type_cols) == 5
        ), "primary_type_cols must contain exactly 5 columns"
        assert len(specialty_cols) == 5, "specialty_cols must contain exactly 5 columns"
        ...
        self.df = self.df.withColumn(
            ...
            array(
                *[
                    struct(
                        col(primary_type_cols[i]).alias("Primary_Type"),
                        col(specialty_cols[i]).alias("Specialty"),
                    )
                    for i in range(5)
                ]
            ),
